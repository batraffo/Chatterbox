...
icl_hash_t * hash;
static coda * codino;
static tabgroup ** tabellagruppi;
static pthread_cond_t codavuota=PTHREAD_COND_INITIALIZER;
static pthread_mutex_t mutecscodaoperazioni=PTHREAD_MUTEX_INITIALIZER;
static pthread_mutex_t mutecslistaconn=PTHREAD_MUTEX_INITIALIZER;
static pthread_mutex_t mutecsfmax=PTHREAD_MUTEX_INITIALIZER;
static pthread_mutex_t mutecstats=PTHREAD_MUTEX_INITIALIZER;
static pthread_mutex_t *locker_gruppi;

volatile sig_atomic_t siguscita_flag;
		...
		switch(su_cui_lavorare->messaggio.hdr.op){
			...
			case POSTFILE_OP:{
				...
				if(msg->hdr.len>MaxFileSize*1024){
					...
					if(su_cui_lavorare->messaggio.data.buf[0]!='.' && su_cui_lavorare->messaggio.data.buf[0]!='/' ){
